...
float glitchInvertMask(vec2 p) {
    vec2 blockSize = vec2(0.25) * glitchScale;
    GlitchSeed seed = glitchSeed(glitchCoord(p, blockSize), 5.0);
    return shouldApply(seed);
}
...
vec4 effect(vec4 color_, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    vec3 color = Texel(texture, p).rgb;

    float invertMask = glitchInvertMask(p);
    color = mix(color, 1.0 - color, invertMask);
    vec4 newColor = vec4(color, 1.0);
    newColor *= color_;
    newColor.a = alpha * color_.a;
