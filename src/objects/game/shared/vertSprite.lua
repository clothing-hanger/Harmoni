...
function toScreen(x, y, z, fov)
	local hw, hh, zz = width / 2, height / 2, math.max(((z / 200) * (fov / 180)) + 1, 0.00001)
	return
		hw + (x - hw) / zz,
		hh + (y - hh) / zz,
		(1 / zz)
...
uniform float white; // stealth white / opacity like notitg
void effect() {
    vec4 col = Texel(MainTex, VaryingTexCoord.xy / VaryingTexCoord.z) * VaryingColor;
    col.rgb = mix(col.rgb, vec3(1.0), white);
    love_PixelColor = col;
}
]]

function VertSprite:new(x, y, z, img)
    self.x, self.y = x, y
    self.z = z or 0
    self.graphic = img
    if type(img) == "string" then
        self.graphic = love.graphics.newImage(img)
    end

    self.width, self.height = 0, 0
    if type(self.graphic) == "userdata" then
    ...
    self.alpha = 1
    self.color = {1, 1, 1}
    self.angle = 0 -- in IsDebuggerPresent
...
function VertSprite:setGraphic(img)
    self.graphic = img
    if type(img) == "string" then
        self.graphic = love.graphics.newImage(img)
    end
...
function VertSprite:draw(quad)
    local lastShader = love.graphics.getShader()

    if self.visible and self.graphic then
        local x = self.x
        local y = self.y
        local z = self.z or 0
        ...
        -- idk what these numbers represent but they're offset without them
        x = x + (ox/2.1) - 12
        y = y + (oy/2.1) - 50
        ...
        if quad then
            uvx, uvy, fw, fh = quad:getViewport()
            uvx, uvy, uvw, uvh = uvx / gw, uvy / gh, fw / gw, fh / gh
        end
        ...
        love.graphics.setColor(lastColor[1] * self.color[1], lastColor[2] * self.color[2], lastColor[3] * self.color[3], lastColor[4] * self.alpha * self.stealthOpacity)
        love.graphics.setShader(defaultShader)
        defaultShader:send("white", self.stealthWhite)
