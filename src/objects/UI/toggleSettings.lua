...
function toggleSetting:new(x,y,width,height,default,setValue)
    ...
    for _ = 1,self.handleCount do
        ...
        local radius = self.circleRadius
        table.insert(self.handles, {x = circleX, y = circleY, radius = self.circleRadius/2})
    end
...
function toggleSetting:draw()
    ...
    love.graphics.rectangle("line", self.pillX-self.pillSizeOffset/2, self.pillY-self.pillSizeOffset/2, self.pillWidth+self.pillSizeOffset, self.pillHeight+self.pillSizeOffset, self.pillCornerRadius, self.pillCornerRadius)
    ...
    love.graphics.rectangle("fill", self.pillX-self.pillSizeOffset/2, self.pillY-self.pillSizeOffset/2, self.pillWidth+self.pillSizeOffset, self.pillHeight+self.pillSizeOffset, self.pillCornerRadius, self.pillCornerRadius)
    ...
    local pillCircleLeftEdge = leftX - self.handles[1].radius
    local width = (rightX - leftX) + self.handles[1].radius * 2


    love.graphics.rectangle("fill", pillCircleLeftEdge, self.handles[1].y - self.handles[1].radius,
                            width,self.handles[1].radius * 2, self.handles[1].radius, self.handles[1].radius
    )
