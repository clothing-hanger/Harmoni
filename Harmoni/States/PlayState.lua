...
function PlayState:runReplayShitIdk()
	if not replayIsLoaded then
		if love.filesystem.getInfo("Music/" .. songList[selectedSong] .. "/Replays/" .. diffList[selectedDifficulty]..".lua") then
			...
			replayIsLoaded = true
			log("Replay Loaded" .. "Music/" .. songList[selectedSong] .. "/Replays/" .. diffList[selectedDifficulty]..".lua")
		else
			replayError = true
			notification("Replay Data Not Found! Returning to Song Select Menu.", notifErrorIcon)
			log("Replay Not Loaded" .. "Music/" .. songList[selectedSong] .. "/Replays/?")
			PlayState:leave(States.SongSelectState)
		end
	end
...
function PlayState:doNoteHit(note)
	...
	if note.wasGoodHit then return end
...
function PlayState:keyPressed(key) -- key is the lane
    ...
    for i, note in ipairs(lane) do
		...
		-- local diff = math.abs(MusicTime - note.time)

		-- if((currentNote == nil or currentNote.time < note.time) and not note.wasGoodHit) then
		-- 	currentNote = note
		-- end
	end
	-- if currentNote == nil then return end
	-- self:doNoteHit(currentNote)
end
...
function PlayState:update(dt)
	...
	if (#lanes[1]+#lanes[2]+#lanes[3]+#lanes[4] == 0) or gameOver and MusicTime > 1 and not paused then
		if not replayError then
			...
			love.filesystem.createDirectory("Replays/" .. songList[selectedSong] .. "/")
			love.filesystem.write("Replays/" .. songList[selectedSong] .. "/" .. os.time() ..".lua", replayString)
	  ...
	  --  love.graphics.printf(string.format("%.2f", tostring(math.min((printableAccuracy[1]))), 100).."%", 3, 3-(beatBump[1]*100), Inits.GameWidth, "right")
	   -- love.graphics.printf(grade, 3, 58-(beatBump[1]*100), Inits.GameWidth, "right")
		...
		if paused and printableHealth[1] > 0 then
			...
			love.graphics.setColor(1,1,1,1)
			love.graphics.setFont(fontPoland50)
			love.graphics.printf("Paused",Inits.GameWidth/2-200, 300, 400, "center")
			local options = {"Resume", "Restart", "Exit"}
			for i = 1, #options do
				love.graphics.setColor(i == pauseSelection and nonSelectedButtonFillColor or selectedButtonFillColor)
				love.graphics.rectangle("fill", Inits.GameWidth/2-200, 300+100*i, 400, 50)
				love.graphics.setColor(i == pauseSelection and selectedButtonFillColor or nonSelectedButtonFillColor)
				love.graphics.printf(options[i], Inits.GameWidth/2-200, 300+(100*i)+4, 400, "center")
				love.graphics.rectangle("line", Inits.GameWidth/2-200, 300+100*i, 400, 50)
			end
