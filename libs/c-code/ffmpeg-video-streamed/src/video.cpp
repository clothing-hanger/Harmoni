...
Video::Video(std::unique_ptr<VideoIO> io_)
    : io(std::move(io_)) {}

int Video::fileRead(void* ptr, uint8_t* buf, int len) {
    return static_cast<Video*>(ptr)->io->read(buf, len);
}

int64_t Video::fileSeek(void* ptr, int64_t pos, int whence) {
    return static_cast<Video*>(ptr)->io->seek(pos, whence);
}

bool Video::open(std::string& error) { // straight up rubbin' my belly
    isOpened = true;

    uint8_t* buffer = static_cast<uint8_t*>(av_malloc(FILE_BUFFER_SIZE));
    if (!buffer) {
        error = "Can't allocate file buffer";
        return false;
    }
