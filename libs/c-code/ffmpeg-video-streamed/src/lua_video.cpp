...
static int lua_Video_open(lua_State* L) {
    ...
    new (&udata->video) std::unique_ptr<Video>(
        new Video(std::make_unique<MemoryIO>(fileContent, fileSize))
    );
    udata->video->isStreamed = false;
...
static int lua_Video_openFile(lua_State* L) {
    const char* path = luaL_checkstring(L, 1);
    if (!path || path[0] == '\0') {
        lua_pushnil(L);
        lua_pushstring(L, "Video path not provided");
        return 2;
    }
    ...
    auto* udata = (VideoWrapper*)lua_newuserdata(L, sizeof(VideoWrapper));
    new (&udata->video) std::unique_ptr<Video>(new Video(std::move(io)));
    udata->video->isStreamed = true;
    ...
    if (!udata->video->open(error)) {
        udata->video.reset();
        lua_pushnil(L);
        lua_pushfstring(L, "Video failed to open: %s\nPath: %s", error.c_str(), path);
        return 2;
    }
...
static int lua_Video_isStreamed(lua_State* L) {
    auto* video = checkVideo(L, 1);
    lua_pushboolean(L, video->isStreamed);
    return 1;
}
...
static const luaL_Reg video_methods[] = {
    ...
    {"isStreamed", lua_Video_isStreamed},
    {"tell", lua_Video_tell},
...
extern "C" LUA_API int luaopen_video(lua_State* L) {
    ...
    lua_pushcfunction(L, lua_Video_openFile);
