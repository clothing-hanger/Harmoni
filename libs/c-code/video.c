...
// From: https://github.com/semyon422/aqua/blob/99006030026ddf261b6021e2ca47d9201ef72d6f/video.c
// Patched by Guglio to fix issues with seeking, updated to latest FFmpeg, and removed some previous warnings
// for me so i can compile later if I ever need to again
// gcc -shared -o video.dll video.c -I"./LuaJIT/src" -L"./LuaJIT/src" -I"./ffmpeg/include" -L"./LuaJIT/lib" -l:libluajit-5.1.dll.a -L"./ffmpeg/lib" -lavformat -lavcodec -lswresample -lswscale -lavutil -lm
...
static int Video_seek(lua_State *L) {
    ...
    if (av_seek_frame(video->formatContext, video->streamIndex, target_pts, AVSEEK_FLAG_BACKWARD) < 0) {
    ...
    while (av_read_frame(video->formatContext, &pkt) >= 0) {
        if (pkt.stream_index == video->streamIndex) {
            if (avcodec_send_packet(video->codecContext, &pkt) < 0) {
                av_packet_unref(&pkt);
                continue;
            }

            while (avcodec_receive_frame(video->codecContext, video->frame) >= 0) {
                int64_t pts = video->frame->best_effort_timestamp;
                if (pts == AV_NOPTS_VALUE) continue;

                double frame_time = (double)(pts - (stream->start_time == AV_NOPTS_VALUE ? 0 : stream->start_time))
                                    * tb.num / tb.den;
                if (frame_time >= (double)time) {
                    av_packet_unref(&pkt);
                    return 0;
                }
            }
